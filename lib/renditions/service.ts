...
import type { Catalog, Locale, Story } from "@/lib/content/types";
...
export type EnsureResult =
  ...
  | {
      status: "unavailable";
      reason: "budget" | "disabled" | "failed" | "unpublished" | "unknown_voice" | "unknown_story";
      message: string;
      retryAfter?: string;
...
export async function listReadyVoices(
  ...
  for (const v of voices.chirpVoices) {
    try {
      const plan = await buildPlan(story, collectionTitleOf(catalog, story), locale, v.name, voices);
...
export async function ensureRenditionForStory(
  ...
  try {
    plan = await buildPlan(story, collectionTitle, locale, voiceName, voices);
  } catch (e) {
...
async function fallbackRendition(
  ...
  try {
    const plan = await buildPlan(story, collectionTitle, locale, def, voices);
